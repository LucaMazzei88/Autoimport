...
public class Driver {
	...
	public void aspetta(String testo, long timeout) throws OutOfTimeException{
		long t0=System.currentTimeMillis();
		long t=t0;
		boolean inTime=true;
		while(!eventDriver.getPageSource().contains(testo) && (inTime=t<(t0+timeout))){
	...
	public void click(By by, String testo, boolean withTimeout) throws OutOfTimeException{
		if(withTimeout)
			aspetta(testo, TIMEOUT);
		else
			aspetta(testo);
		eventDriver.findElement(by).click();
	}
	public void clickByHref(By by, String testo,boolean withTimeout) throws OutOfTimeException{
		if(withTimeout){
			aspetta(testo, TIMEOUT);	
		}
		else
